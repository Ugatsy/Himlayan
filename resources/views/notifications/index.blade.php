                    ...
                    @forelse($notifications as $notification)
                        <div class="flex items-start gap-4 py-4 border-b border-gray-100 last:border-0 @if(!$notification->is_read) bg-indigo-50/50 -mx-6 px-6 @endif">
                            <div class="shrink-0 mt-1">
                                <span class="w-2.5 h-2.5 rounded-full block @if($notification->type === 'burial_reminder') bg-blue-1000 @elseif($notification->type === 'installment_due') bg-yellow-1000 @elseif($notification->type === 'overdue') bg-red-1000 @else bg-gray-400 @endif @if(!$notification->is_read) ring-2 ring-offset-1 ring-indigo-400 @endif"></span>
                            </div>
