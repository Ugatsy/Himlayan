            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                <div class="p-6 border-b border-gray-100 flex flex-wrap items-center justify-between gap-4">
                    <div class="flex items-center gap-4">
                        <span class="text-gray-500">{{ $burials->count() }} burials</span>
                        <input type="text" id="search-burials" placeholder="Search by deceased name or plot…" class="rounded-lg bg-white border-gray-200 text-gray-900 placeholder-gray-500 text-sm shadow-sm focus:border-brand-blue focus:ring-brand-blue w-64">
                    </div>
                    <div class="flex gap-2">
                        <a href="{{ route('deceased.index') }}" class="text-brand-blue hover:text-brand-blue-dark text-sm font-medium px-3 py-2">Deceased Registry</a>
                        <a href="{{ route('burials.create') }}" class="bg-brand-blue text-white px-4 py-2 rounded-xl hover:bg-brand-blue-dark text-sm">+ Schedule Burial</a>
                    </div>
                </div>
                <div class="p-6">
                    <table class="w-full text-sm">
                        <thead><tr class="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider"><th class="py-2 font-medium">Deceased Name</th><th class="font-medium">Plot</th><th class="font-medium">Client</th><th class="font-medium">Date</th><th class="font-medium">Status</th><th class="font-medium"></th></tr></thead>
                        <tbody id="burials-table-body">
                            @forelse($burials as $burial)
                                <tr class="border-b hover:bg-blue-50/50 burial-row">
                                    <td class="py-3"><a href="{{ route('burials.show', $burial) }}" class="text-brand-blue hover:text-brand-blue-dark font-medium deceased-name">{{ $burial->deceased_name }}</a></td>
                                    ...
                                    <td class="text-right">
                                        @if($burial->burial_status === 'scheduled')
                                            <form method="POST" action="{{ route('burials.approve', $burial) }}" class="inline">
                                                @csrf @method('PATCH')
                                                <button type="submit" class="text-green-600 hover:text-green-700 mr-3 font-medium">Approve</button>
                                            </form>
                                        @endif
                                        <a href="{{ route('burials.edit', $burial) }}" class="text-brand-blue hover:text-brand-blue-dark mr-3">Edit</a>
                                        <button type="button" class="text-red-500 hover:text-red-700 delete-btn" data-url="{{ route('burials.destroy', $burial) }}" data-label="Delete this burial record?">Delete</button>
                                    </td>
                                </tr>
                            @empty
                                <tr id="empty-row"><td colspan="6">
                                    <div class="py-12 text-center">
                                        ...
                                        <a href="{{ route('burials.create') }}" class="inline-block mt-4 bg-brand-blue text-white px-4 py-2 rounded-xl hover:bg-brand-blue-dark text-sm">+ Schedule Burial</a>
                                    </div>
                                </td></tr>
                            @endforelse
                        </tbody>
