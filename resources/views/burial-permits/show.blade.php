    ...
    <div>
        <div class="max-w-3xl mx-auto">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
                <div class="border-2 border-dashed border-gray-100 p-8 rounded-lg">
                    ...
                    <dl class="grid grid-cols-2 gap-4 text-sm">
                        <div class="col-span-2">
                            <dt class="text-gray-500">Permit Number</dt>
                            <dd class="font-mono font-bold text-lg">{{ $burialPermit->permit_number }}</dd>
                        </div>
                        <div>
                            <dt class="text-gray-500">Deceased Name</dt>
                            <dd class="font-medium">{{ $burialPermit->deceased_name }}</dd>
                        </div>
                        <div>
                            <dt class="text-gray-500">Date of Death</dt>
                        ...
                        @endif
                        <div>
                            <dt class="text-gray-500">Death Certificate #</dt>
                        ...
                        <div>
                            <dt class="text-gray-500">Plot</dt>
                            <dd class="font-medium">{{ $burialPermit->contract->plot?->plot_number ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-gray-500">Permit Fee</dt>
                            <dd class="font-medium">₱{{ number_format($burialPermit->burial_permit_fee, 2) }}</dd>
                        </div>
                        <div>
                            <dt class="text-gray-500">Status</dt>
                            <dd>
                                <span class="px-2 py-1 text-xs font-semibold rounded-full @if($burialPermit->status === 'issued') bg-brand-blue/10 text-brand-blue @elseif($burialPermit->status === 'used') bg-green-100 text-green-700 @else bg-gray-100 text-gray-600 @endif">{{ $burialPermit->status }}</span>
                            </dd>
                        </div>
                        <div>
                ...
                <div class="mt-4 flex gap-4">
                    <a href="{{ route('burial-permits.edit', $burialPermit) }}" class="text-brand-blue hover:text-brand-blue-dark">Edit</a>
                    <a href="{{ route('contracts.show', $burialPermit->contract) }}" class="text-brand-blue hover:text-brand-blue-dark">View Contract</a>
                    <button type="button" class="text-red-500 hover:text-red-700 delete-btn" data-url="{{ route('burial-permits.destroy', $burialPermit) }}" data-label="Delete this burial permit?">Delete</button>
                </div>
            </div>
        </div>
    </div>
