<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
        <div class="text-sm font-medium text-gray-500">Contracts Pending Your Signature</div>
        <div class="mt-2 text-3xl font-bold text-amber-600">{{ $pendingMayor }}</div>
        <div class="mt-1 text-sm">
            <a href="{{ route('contracts.index') }}" class="text-indigo-600 hover:text-indigo-900">Review for approval</a>
        </div>
    </div>
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
        <div class="flex items-center justify-between mb-4">
            <h3 class="font-semibold text-gray-900">Contracts Awaiting Mayor</h3>
            <a href="{{ route('contracts.index') }}" class="text-sm text-indigo-600 hover:text-indigo-900">View all</a>
        </div>
        @php
            $pendingContracts = \App\Models\Contract::whereNotNull('approved_by_treasurer_at')
                ->whereNull('approved_by_mayor_at')
                ->with('client')
        ...
        <div class="space-y-3">
            @forelse($pendingContracts as $contract)
                <div class="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                    ...
                    <a href="{{ route('contracts.show', $contract) }}" class="text-sm text-indigo-600 hover:text-indigo-900">Review & Sign</a>
                </div>
            @empty
                <p class="text-sm text-gray-500">No contracts pending your signature.</p>
            @endforelse
        </div>
    </div>
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
