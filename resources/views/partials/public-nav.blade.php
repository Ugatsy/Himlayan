<nav class="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm shadow-sm" x-data="{ mobileOpen: false }">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16 lg:h-20">
            ...
            <div class="hidden lg:flex items-center gap-8">
                ...
                <a href="{{ route('public.columbarium') }}" class="text-sm font-medium text-gray-600 hover:text-emerald-800 transition-colors">Columbarium</a>
                ...
                @else
                    ...
                    @if (Route::has('register'))
                        <a href="{{ route('register') }}" class="inline-flex items-center px-4 py-2 border border-emerald-800 text-emerald-800 text-sm font-medium rounded-lg hover:bg-emerald-50 transition-colors">Register</a>
                    @endif
                @endauth
            </div>
    ...
    <div x-show="mobileOpen" class="lg:hidden border-t bg-white" x-cloak>
        <div class="px-4 py-4 space-y-3">
            ...
            <a href="{{ route('public.columbarium') }}" @click="mobileOpen = false" class="block text-sm font-medium text-gray-600 hover:text-emerald-800">Columbarium</a>
            ...
            @else
                ...
                <a href="{{ route('register') }}" @click="mobileOpen = false" class="block text-center px-4 py-2 border border-emerald-800 text-emerald-800 text-sm font-medium rounded-lg">Register</a>
            @endauth
        </div>
    </div>
</nav>
